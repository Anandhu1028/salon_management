...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize custom country code dropdowns
    window.initCountryCodeDropdowns();
});

// Custom country code dropdown controller
function initCountryCodeDropdowns() {
    document.querySelectorAll('[data-code-dropdown]').forEach(wrapper => {
        if (wrapper.dataset.initialized) return;
        wrapper.dataset.initialized = 'true';

        const btn = wrapper.querySelector('.phone-prefix-btn');
        const valSpan = wrapper.querySelector('.phone-prefix-val');
        const hiddenInput = wrapper.querySelector('input[type="hidden"]');
        const searchInput = wrapper.querySelector('.phone-prefix-search-input');
        const items = wrapper.querySelectorAll('.phone-prefix-item');
        ...
        btn.addEventListener('click', (e) => {
            ...
            document.querySelectorAll('[data-code-dropdown].is-open').forEach(el => el.classList.remove('is-open'));
            if (!isOpen) {
        ...
        items.forEach(item => {
            item.addEventListener('click', (e) => {
                ...
                if (valSpan) valSpan.textContent = code;
...
window.initCountryCodeDropdowns = initCountryCodeDropdowns;

window.setCountryCodeValue = (hiddenInputId, code) => {
    const input = document.getElementById(hiddenInputId);
    if (!input) return;
    const targetCode = code || '+91';
    input.value = targetCode;
    const wrapper = input.closest('[data-code-dropdown]');
    if (wrapper) {
        const valSpan = wrapper.querySelector('.phone-prefix-val');
        if (valSpan) valSpan.textContent = targetCode;
        const items = wrapper.querySelectorAll('.phone-prefix-item');
        items.forEach(it => {
            if (it.dataset.code === targetCode) {
...
document.addEventListener('click', (e) => {
    if (!e.target.closest('[data-code-dropdown]')) {
        document.querySelectorAll('[data-code-dropdown].is-open').forEach(el => el.classList.remove('is-open'));
    }
});
